...
<?php 
	...
	echo '<div class="container green">
		<form method="POST" action="" class="mt-5 p-4">
			<h2 class=" text-white">Rapport 1</h2>
			<p class="text-white">U kunt hier een kostenoverzicht bekijken van de kosten per consultant per project.</p>
			<div class="form-group row">
				<div class="col-sm-12">
					<p class="text-white">Consultant:</p><select class="form-control col-sm-12" name="consultant" required>';
						echo '<option></option>';
							$query = "SELECT naam FROM gebruikers WHERE id != 1";
	...
	if(isset($_POST["submit"]) ) {
		...
		$naam = $_SESSION["naam"];
		$query = "SELECT projectkosten.consultant,projectkosten.project,kostenomschrijving.kostencode,projectkosten.omschrijving,projectkosten.datum,projectkosten.kosten FROM projectkosten INNER JOIN kostenomschrijving ON projectkosten.omschrijving = kostenomschrijving.omschrijving WHERE projectkosten.consultant = '$consultant' ORDER BY projectkosten.consultant, projectkosten.project, projectkosten.kostencode";
		$query2 = "SELECT SUM(kosten) FROM projectkosten WHERE consultant = '$consultant'";
